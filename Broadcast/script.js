...
function setup() {
  ...
  // background(0);
  // mobilenet = ml5.featureExtractor('MobileNet', modelReady);
  // classifier = mobilenet.classification(face, videoReady);
...
function gotResults(error, result) {
  if (error) {
    ...
    classifier.classify(gotResults);
    console.log("classifier.classify(gotResults)",classifier.classify(gotResults));
    console.log("label = result[0].label",label = result[0].label);
