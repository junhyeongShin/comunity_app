...
$clan_id = $_GET['clan_id'];
  ...
  $query_get_user_member ="SELECT member FROM Clan WHERE id = $clan_id ";

  //DB에 연결합니다.
  $conn = mysqli_connect($db_address, $db_userid, $db_userpw);
  mysqli_select_db($conn, $database);
  $result = $conn->query($query_get_user_member);
  ...
  for($i=0;$i<count($array);$i++){
    ...
    if(count($test_array)>0){
      ...
      $query_update_clan ="UPDATE Clan SET member = '$update' WHERE id = $clan_id ";

        //DB에 연결합니다.
      $conn = mysqli_connect($db_address, $db_userid, $db_userpw);
      mysqli_select_db($conn, $database);
      $result = $conn->query($query_update_clan);

      if($result){
        echo 'success';

      }else{
        echo $query_update_clan;
      }
